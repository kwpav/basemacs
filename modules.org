...
* About
This is a set of "modules" for =basemacs=. As with the rest of =basemacs=, these are only a beginning point. They can be used as is, but they are intended to be examples of some of the cool packages and features that Emacs has to offer.
